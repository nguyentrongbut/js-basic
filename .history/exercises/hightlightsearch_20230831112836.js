    ...
    function handleHighlight(e) {
        ...
        [...items].forEach((item) => {
            ...
            if (index >= 0 && text.toLowerCase().startsWith(filter.charAt(0))) {
                ...
                console.log(`text.substring(index,index + filter.length): ${text.substring(index,index + filter.length)}`);
                console.log(`text.substring(index,index + filter.length): ${text.substring(index + filter.length)}`);
